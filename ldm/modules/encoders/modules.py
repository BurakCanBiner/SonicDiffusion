...
import sys
sys.path.append('/kuacc/users/bbiner21/hpc_run/Github/AudioCLIP')
...
class AudioCLIPEncoder(AbstractEncoder):
    def __init__(self, device="cuda",ckpt_path = "/kuacc/users/bbiner21/hpc_run/Github/AudioCLIP/assets/AudioCLIP-Full-Training.pt"):
    ...
    def encode_image(self, image):
        ((_, image_features, _), _), _ = self.model(image=image)

        return image_features.unsqueeze(1)    
    
    def encode_text(self, text):
        ((_, _, text_features), _), _ = self.model(text=text)
